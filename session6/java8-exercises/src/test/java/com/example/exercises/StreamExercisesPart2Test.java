...
public class StreamExercisesPart2Test {
    ...
    public void testMinAndMaksAmount() throws Exception {
        ...
        Optional<Integer> max = stream1.skip(1).map(Integer::parseInt).max(Comparator.naturalOrder());
        Optional<Integer> min = stream2.skip(1).map(Integer::parseInt).min(Comparator.naturalOrder());;
